...
\item[RQ1 (Carrier Source: Real vs.\ ML)] Does carrier source (real photographs vs.\ ML-generated images) significantly affect steganographic detectability under matched embedding settings?

\item[RQ2 (Generation-Model Effect Within ML)] Within ML-generated carriers, does the choice of text-conditional generation model (ML-A vs.\ ML-B) significantly affect detectability under prompt-matched conditions?

\item[RQ3 (Payload Interaction)] Does payload size alter source-related detectability gaps (real vs.\ ML and ML-A vs.\ ML-B)?

\item[RQ4 (Embedding Method)] Do different embedding methods (spatial LSB vs.\ frequency-domain DCT) interact differently with carrier source in terms of detectability?

\item[RQ5 (Encryption Interaction)] Does payload encryption prior to embedding change detectability, and does this effect depend on carrier source?
...
\textbf{Real images (500 total).} To enforce caption quality, we use only datasets that already provide detailed human-written captions per image: \textbf{COCO}~\cite{coco2014} (300 images) and \textbf{Flickr30k}~\cite{flickr30k2014} (200 images). All selected images are normalized to 512$\times$512, RGB, 8-bit PNG.

\textbf{Caption curation.} Each selected real image is assigned one complete, relevant caption (subject + key scene context), chosen from the dataset-provided human captions only. No generated captions are used. Images with ambiguous, underspecified, or low-information captions are excluded.

\textbf{ML-generated images (1{,}000 total).} From the curated 500-image real set, we generate two prompt-matched sets: \textbf{ML-A} (500) with Stable Diffusion XL 1.0~\cite{sdxl2023} and \textbf{ML-B} (500) with PixArt-$\alpha$~\cite{pixart2023}, both via \texttt{diffusers}. This intentionally compares two different generator families (latent U-Net diffusion vs.\ diffusion-transformer). Each real-image caption is used once per model, yielding paired prompts.

\textbf{Generation-stage quality control.} Generated candidates are filtered before embedding using a BRISQUE gate plus artifact rejection, then normalized to a single 512$\times$512 RGB PNG format used by all embedding and detection branches.

\subsection{Embedding Methods}

We implement two canonical steganographic methods spanning the two principal domains:

\textbf{LSB substitution (spatial domain)} replaces the $k$ least significant bits of each pixel channel value with pseudorandom message bits, using a PRNG-keyed pixel selection mask. We test $k\!=\!1$ (Low, Medium payload) and $k\!=\!2$ (High payload).
...
\]
where the sign encodes the message bit. DCT embedding is applied directly to the same PNG-format carriers (no format conversion), so comparisons with LSB are not confounded by codec differences. It is chosen alongside LSB to test whether frequency-domain methods are more sensitive to carrier source (RQ4), since DCT coefficients reflect the generative model's learned spectral distribution directly.

\subsection{Encryption}

To test RQ5, payloads are processed under two conditions before embedding: plain and AES-256-CBC encrypted. The same encryption setting is applied uniformly across both LSB and DCT embedding branches, so any detectability difference can be attributed to encryption status rather than method-specific preprocessing.

\subsection{Image-Quality Tests (Quality Control)}

Image quality is treated as a validity control at generation, post-embedding, and post-encryption stages. For each stego/cover pair we compute PSNR, SSIM, and FSIM; BRISQUE is used as a no-reference generation gate. Metrics are reported by source group, embedding method, and payload level to rule out trivial degradation effects.
...
\midrule
SRM+EC~\cite{fridrich2012srm,kodovsky2012ec} & Classical ML & Labeled & LSB + DCT \\
...
\textbf{SRM + Ensemble Classifier (EC)}~\cite{fridrich2012srm,kodovsky2012ec} is the primary classical baseline used in all conditions. We extract full SRM residual co-occurrence features and train an ensemble of FLD base learners on bootstrap-resampled feature subspaces (the canonical SRM+EC setup), using source-stratified train/test splits to avoid leakage.

\textbf{RS + $\chi^2$ analytical checks}~\cite{fridrich2001lsb,westfeld1999chi} are applied only to LSB experiments as fast, training-free detectors.

\textbf{Block-DCT shift test}~\cite{wangmoulin2003dct} is applied only to DCT experiments as a training-free counterpart to RS. It measures embedding-induced shifts in block-DCT coefficient distributions and quantization residues relative to matched cover statistics.
...
\textbf{Exp.\,1 (RQ1: Carrier Source, real vs.\ pooled ML).} Apply all applicable detectors to all 1{,}500 carriers across payload levels, methods, and encryption conditions (LSB: SRM+EC, RS, plus $\chi^2$ sensitivity; DCT: SRM+EC, block-DCT shift test). Compute AUC per source group and compare real vs.\ pooled ML (ML-A + ML-B) performance under matched settings using Wilcoxon signed-rank and Cohen's~$d$.

\textbf{Exp.\,2 (RQ2: Generation-model effect within ML).} Using prompt-matched ML pairs, compare AUC between ML-A and ML-B under identical embedding/encryption/payload settings. Test whether generation model induces a significant detectability difference, with detector-specific reporting and effect sizes.

\textbf{Exp.\,3 (RQ3: Payload interaction).} Plot AUC and source-contrast gaps across Low/Medium/High payload. Test source $\times$ payload interaction and evaluate whether payload modulates both real-vs-ML and ML-A-vs-ML-B detectability contrasts.

\textbf{Exp.\,4 (RQ4: Embedding-method interaction).} Run ANOVA on AUC with factors source category and embedding method (LSB/DCT). A significant source $\times$ method interaction indicates method-dependent detectability differences across source groups.

\textbf{Exp.\,5 (RQ5: Encryption interaction).} For each source group and embedding method, compare AUC between plain and AES-256-CBC payload conditions. Test both the encryption main effect and source $\times$ encryption interaction.
...
The closest related study is De et al.~\cite{de2022ai}, which demonstrates AI-generated photorealistic images for steganographic secret sharing. However, it does not perform a controlled real-versus-ML comparison under identical embedding pipelines, does not compare multiple generation models under prompt-matched conditions, and does not evaluate detectability using classical steganalysis endpoints such as ROC-AUC. Our factorial design (source category $\times$ embedding method $\times$ payload size $\times$ encryption) makes steganalysis detectability the primary outcome.
...
Detailed Gantt charts are provided in Appendix~\ref{app:gantt}.
...
To satisfy the minimal passing criteria, the study must implement at least one encryption scheme, two embedding approaches (one spatial-domain and one frequency-domain), and domain-appropriate steganalysis detectors for the implemented embedding methods, and it must provide conclusive answers to RQ1 (carrier-source effect: real vs. ML) and RQ2 (generation-model effect within ML)
...
in \textit{Proc.\ IEEE SSP}, pp.~477--488, 2003.
...
\hline
3.2 & SRM + EC and DCT statistical detector setup & 14.04 & 01.05 & 14 &
